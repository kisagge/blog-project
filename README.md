...
- **공개 피드**: 글 목록 + 상세(마크다운 렌더), **제목·내용·요약 검색**, **무한스크롤(10개 단위)**, 조회수 집계
...
- **회원**: 가입 신청 → 관리자 승인 → 로그인(JWT 세션), 비밀번호 재설정(이메일 6자리 코드), **내 정보(닉네임 수정)**
- **댓글·좋아요**: 2뎁스 댓글(답글) + 댓글/피드 좋아요, 비회원은 로그인 유도 모달
- **알림**: 인앱 알림 센터(읽음 처리·딥링크 하이라이트) + **웹 푸시**(댓글/답글)
- **PWA**: 설치형 + 오프라인(서비스 워커·manifest·아이콘)
- **SNS 공유**: X · 카카오톡 · 네이티브 공유 · URL 복사 (og:image 포함)
- **관리자 CMS**: 글·던파 캐릭터 CRUD, 회원 승인/거절, 본문 이미지 업로드, 공개 범위 모달, 점검 모드
