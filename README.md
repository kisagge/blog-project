...
- **게임**(`/play`, 회원·관리자): 로그라이크 텍스트 RPG **심연 강하** — 시드 결정론 PRNG·절차생성으로 던전을 한 층씩 내려가며 전투·보물·상점·휴식·함정·보스를 헤쳐 최대한 깊이 도달(영구사망). 순수 엔진(`lib/game/rogue/`) + 텍스트 UI, **같은 시드 = 같은 던전**(재현·도전), **회원 리더보드**(런 종료 시 점수 기록, 서버 재계산). 키보드 전조작·`aria-live` 로그
