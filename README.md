...
- **게임**(`/play`): 회원 전용 **턴제 SRPG**(에테르 택틱스) — three.js 2.5D 보드. 순수 게임 로직(`lib/game/srpg/`)과 렌더 분리, 캔버스는 `next/dynamic`로 지연 로드. **플레이 가능한 단일 전투**(마우스·키보드 전조작, 선택→이동→공격→적 AI 턴→승패, `aria-live`·스크린리더 보드 미러)
