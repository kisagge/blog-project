...
describe("df-characters", () => {
  ...
  test("신규는 기본 비공개 + setFeaturedVisibility로 변경", async () => {
    const list = await m.listFeatured();
    const id = list[0].id;
    expect(list[0].visibility).toBe("private");
    await m.setFeaturedVisibility(id, "public");
    expect((await m.listFeatured())[0].visibility).toBe("public");
    await m.setFeaturedVisibility(id, "members");
  ...
  test("listFeaturedVisible: 권한별 필터(비공개 제외)", async () => {
    ...
    const [a, b] = all;
    await m.setFeaturedVisibility(a.id, "public");
    await m.setFeaturedVisibility(b.id, "members");
