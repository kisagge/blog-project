...
DB 로직은 prisma 호출을 mock하면 동어반복이 되므로, **임시 SQLite에 실제 쿼리를 돌려** 검증하는 통합 테스트 헬퍼(`lib/test-db.ts`)를 만들어 페이지네이션·검색·접근 제어·승인 흐름·댓글 깊이·좋아요·알림·속도 제한·신고까지 커버. 전체 **17 → 235 테스트**로 확장.
...
- 통합 테스트 도입(17 → 235), CI에서 타입체크·린트·테스트·이미지 빌드 게이트
