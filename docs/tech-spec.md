...
- **접근 제어·보안**: `/play`는 `getViewerRole()`가 `anon`이면 `MemberGate`(회원 전용). 플레이스홀더 스프라이트는 **코드 생성 `CanvasTexture`**(진영 색+모양으로 색맹 안전, 클래스 글리프)라 외부 에셋·**CSP 변경 불필요**(번들 three=`self`, 텍스처=`data/blob`).
- **상호작용·접근성(S3)**: 플레이 흐름을 **순수 컨트롤러**(`app/play/controller.ts` — 선택→이동/공격→대기/턴종료, three/DOM 0)로 분리해 단위 테스트하고, three 씬은 그 상태를 그리고 **마우스 raycast 픽**만 위로 올린다. **키보드 전조작**(`role="application"` 래퍼 + 방향키 커서·Enter 활성·Esc 해제·E 턴종료), `aria-live` 상태(커서·피해·턴)·승패 알림, **SR 보드 미러**(`sr-only` 유닛 목록), 결과 오버레이 포커스 이동. 적 턴은 `runAshenPhase`(결정론). 상태 변경마다 **동적 그룹만 갱신**(렌더러·카메라는 영속). 트윈 없는 즉시 갱신이라 `prefers-reduced-motion` 기본 충족(애니메이션은 다음 단계).
- **카메라·뷰**: 완화된 3/4 원근(fov 35°·피치 ~55°)을 **보드 바운딩에 맞춰 거리 자동 계산**, 리사이즈마다 `setSize`(캔버스 CSS를 컨테이너에 맞춰 **화면 잘림 방지** — S2의 `updateStyle=false` 잔상 수정) + 재프레이밍.
- **폴리시(S4)**: **데미지 프리뷰**(선택 후 적에 커서/호버 시 예상 피해·반격을 HUD·`aria-live`에 — `resolveAttack`를 시뮬레이션해 HP를 diff하므로 실제 전투와 항상 일치, 순수·테스트), **온보드 HP 바**(유닛 머리 위 비율 바, 색이 녹>황>적), **다중 맵 + 선택**(맵을 `lib/game/srpg/maps/`에 데이터로 두고 레지스트리·피커 — `loadMap`·스폰 통행성 테스트로 가드). 지형이 더 이상 정적이 아니라 맵 변경 시 타일을 재구성·재프레이밍(렌더러·카메라는 영속).
- **현황**: S1(순수 엔진)·S2(정적 캔버스)·S3(플레이 가능한 전투)·**S4(데미지 프리뷰·HP 바·다중 맵)** 완료. 다음은 애니메이션 트윈·효과음·확률 레이어·영속/리더보드·맵 에디터(S5+).
...
- 통합 테스트 도입(17 → 592), CI에서 타입체크·린트·테스트·이미지 빌드 게이트
