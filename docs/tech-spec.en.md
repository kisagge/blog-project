...
A new genre, built incrementally with a consistent **logic/render separation** (game rules as pure functions → unit-tested; rendering only draws that state). The first attempt, a **three.js turn-based SRPG** (Aether Tactics), reached S1–S4 but was discontinued and archived (`docs/games/archive/srpg-design.md`) — that genre leans heavily on content (maps/balance/story), a poor fit at hobby scale; the `three` dependency and the `app/play/`·`lib/game/srpg/` code were removed (preserved in git history). It has pivoted to a lighter **roguelike text RPG** (`docs/games/text-rpg-design.md`) — a pure engine + procedural generation + seeded determinism for self-replenishing content, with a member leaderboard (Prisma). Implementation resumes from the next slice (S1 = pure engine + tests).
