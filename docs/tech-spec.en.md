...
- **Access & security**: `/play` shows `MemberGate` when `getViewerRole()` is `anon` (member-only). Placeholder sprites are **code-generated `CanvasTexture`s** (color-blind-safe faction color+shape, class glyph), so no external assets and **no CSP change** (bundled three = `self`, textures = `data/blob`).
- **Interaction & a11y (S3)**: the play flow is a **pure controller** (`app/play/controller.ts` — select→move/attack→wait/end-turn, zero three/DOM) that's unit-tested; the three scene just draws that state and lifts up only the **mouse raycast pick**. **Full keyboard control** (`role="application"` wrapper + arrow-key cursor, Enter to act, Esc to deselect, E to end turn), `aria-live` status (cursor/damage/turn) and result alerts, an **SR board mirror** (`sr-only` unit list), and result-overlay focus management. The enemy turn runs `runAshenPhase` (deterministic). Each state change **rebuilds only the dynamic groups** (renderer/camera persist). Instant updates (no tweens) satisfy `prefers-reduced-motion` by default (animations come next).
- **Camera/view**: a gentler 3/4 perspective (fov 35°, pitch ~55°) with **distance auto-fit to the board bounds**, re-`setSize` on resize (matching the canvas CSS to its container to **prevent clipping** — fixing S2's `updateStyle=false` glitch) + re-framing.
- **Polish (S4)**: **damage preview** (with a unit selected, hovering/cursoring an enemy shows predicted damage + counter in the HUD and `aria-live` — it simulates `resolveAttack` and diffs HP, so it always matches real combat; pure and tested), **on-board HP bars** (a ratio bar above each unit, green→yellow→red), and **multiple maps + a picker** (maps live as data under `lib/game/srpg/maps/` with a registry, guarded by `loadMap`/spawn-passability tests). Terrain is no longer static, so a map change rebuilds the tiles and re-frames the camera (renderer/camera persist).
- **Status**: S1 (pure engine), S2 (static canvas), S3 (playable battle), and **S4 (damage preview, HP bars, multiple maps)** done; next is animation tweens, SFX, a probability layer, persistence/leaderboard, and a map editor (S5+).
...
- Introduced integration tests (17 → 592); CI gates on typecheck, lint, test, and image build
